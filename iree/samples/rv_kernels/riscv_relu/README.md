...
2. Build the `riscv_relu_exec` CMake target to create the static demo. This
target has several dependencies that will translate `riscv_relu.mlir` into a
static library (`riscv_relu.h` & `riscv_relu.c`) as well as a bytecode module
(`riscv_relu.vmfb`) which are finally built into the demo binary:
...
3. Run the sample binary:
  ...
  $QEMU_BIN -cpu rv64,x-v=true,x-k=true,vlen=256,elen=64,vext_spec=v1.0 -L $RISCV_TOOLCHAIN_ROOT/sysroot ./riscv_relu_exec
