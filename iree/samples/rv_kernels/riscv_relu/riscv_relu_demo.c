...
#include "iree/samples/rv_kernels/riscv_relu/riscv_relu_c.h"
...
#include "iree/samples/rv_kernels/riscv_relu/riscv_relu.h"
...
iree_status_t Run() {
  ...
  const struct iree_file_toc_t* module_file_toc =
      iree_samples_static_library_riscv_relu_create();
  ...
  float kInput[] = {4.0f, -2.0f, -4.0f, 0.8f};
  float kExpectedOutput[] = {4.0f, 0.0, 0.0f, 0.8f};
  ...
  float const kMarginOfError = 0.00001;
  if (iree_status_is_ok(status)) {
...
int main() {
  ...
  printf("static lib relu run pass\n");
  return 0;
}
