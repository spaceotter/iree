...
#include "iree/samples/rv_kernels/riscv_sum/riscv_sum_c.h"
...
#include "iree/samples/rv_kernels/riscv_sum/riscv_sum.h"
...
iree_status_t Run() {
  ...
  const struct iree_file_toc_t* module_file_toc =
      iree_samples_static_library_riscv_sum_create();
  ...
  const int kOutputCount = 1;
  iree_hal_dim_t shape[1] = {kElementCount};
  iree_hal_buffer_view_t* arg0_buffer_view = NULL;
  float kFloat4[] = {3.0f, 5.0f, 2.0f, 23.2f};
  ...
  if (iree_status_is_ok(status)) {
    if (mapped_memory.contents.data_length / sizeof(float) != kOutputCount) {
...
int main() {
  ...
  printf("static lib sum run pass\n");
  return 0;
}
