...
#include "iree/samples/rv_kernels/riscv_tanh/riscv_tanh_c.h"
...
#include "iree/samples/rv_kernels/riscv_tanh/riscv_tanh.h"
...
iree_status_t Run() {
  ...
  const struct iree_file_toc_t* module_file_toc =
      iree_samples_static_library_riscv_tanh_create();
  ...
  if (iree_status_is_ok(status)) {
    ...
    for (iree_host_size_t i = 0;
      ...
      if (fabs(data[i] - 0.999329) > kMarginOfError) {
...
int main() {
  ...
  printf("static lib tanh run pass\n");
  return 0;
}
