...
#include "iree/samples/rv_kernels/riscv_transpose/riscv_transpose_c.h"
...
#include "iree/samples/rv_kernels/riscv_transpose/riscv_transpose.h"
...
void init_random_matrix(float* matrix, size_t M, size_t N) {
  for(int j = 0; j < N; j++) {
    for (int i = 0; i < M; i++) {
      matrix[j * N + i] = ((float) rand() / (float) RAND_MAX);
    }
  }
}
...
iree_status_t Run() {
  ...
  const struct iree_file_toc_t* module_file_toc =
      iree_samples_static_library_riscv_transpose_create();
  ...
  // Populate initial values for 4.
  int MDIM = 2;
  int NDIM = 4;
  const int kElementCount = MDIM * NDIM;
  iree_hal_dim_t shape[2] = {MDIM,NDIM};
  float *arg0 = (float *) malloc(MDIM * NDIM * sizeof(float));
  init_random_matrix(arg0, MDIM, NDIM);
  ...
  if (iree_status_is_ok(status)) {
    iree_hal_buffer_view_clone_heap_buffer(
        ...
        iree_make_const_byte_span((void*)arg0,
  ...
  if (iree_status_is_ok(status)) {
    ...
    for (iree_host_size_t i = 0;
      ...
      if (data[i] != arg0[(NDIM * (i % MDIM)) + (i / MDIM)]) {
...
int main() {
  ...
  printf("static lib transpose run pass\n");
  return 0;
}
